...
class Ingest(mainArgs: Array<String>) :
  ...
  private fun jsonToSolrInputDoc(objectNode: ObjectNode): SolrInputDocument {
    val doc = SolrInputDocument()

    doc.addField("id", objectNode["id_str"].textValue().toLong())

    val createdAtInst = objectNode["created_at"].textValue().let { str ->
      LocalDateTime.parse(str, createdAtDateFormat).toInstant(ZoneOffset.UTC)
    }
  ...
  private fun pathTokenizeForFaceting(parts: List<String>): List<String> {
